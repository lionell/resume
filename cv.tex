...
\section{Education}
  \resumeSubHeadingListStart
    \resumeSubheading
      ...
      {M.S. in Computer Science; GPA: 3.93; Est. grad: 2025 (part-time)}{}
    \resumeSubheading
      {Taras Shevchenko National University}{Kyiv, Ukraine}
      {M.S. in Computer Science; GPA: 3.94}{}
    \resumeSubheading
      {Taras Shevchenko National University}{Kyiv, Ukraine}
      {B.S. in Computer Science;  GPA: 3.98}{}
...
\section{Experience}
  \resumeSubHeadingListStart
	...
	\resumeItem{Evaluated Reinforcement Learning approaches for solving abovementioned problem and prototyped the best one.}
	\resumeItem{Performed multiple one-off experiments that identified problematic non-cross platform code in the codebase.}
	\resumeItem{Attempted to learn neural embeddings for the Google's build graph that would work as distance oracles.}
	\resumeItem{Anticipated poor system integration due to volatility of core metrics used for control.}
	\resumeItem{Prototyped time-series forecasting (SARIMA) solution to the abovementioned problem.}
	\resumeItem{Joined Continuous Integration \& Release Infrastructure (CI\&R Infra) team.}
        \resumeItem{Researched online log clustering problem and developed algorithm that outperformed state-of-the-art LogParser.}
        \resumeItem{Performed data modeling around A/B experimentation framework that GCP uses to track VM experiments.}
	\resumeItem{Incorporated various data sources (like Linux kernel subsystems and in-house hypervisor) into VM pipelines.}
	\resumeItem{Joined GCE VMX Tools and Telemetry team responsible for core services on top of hypervisor and tooling around.}
      \resumeItemListEnd
